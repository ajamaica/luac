...
char * getValor(char ** tokenizados , int i, int j){
	char aux [20];
	printf("Concatenando %c i:%d j:%d \n",tokenizados[i][j],i,j);
	if(tokenizados[i][j] >= 48 && tokenizados[i][j] <= 57){
		while(tokenizados[i][j] >= 48 && tokenizados[i][j] <= 57 && tokenizados[i][j] != NULL){
		...
		while(tokenizados[i][j] >= 65 && tokenizados[i][j] <= 122 && tokenizados[i][j] != NULL){
			printf("Concatenando id %c \n",tokenizados[i][j]);
			strcat(aux,tokenizados[i][j]);
			j++;
		}
		int y = 0;
		while (y < 8){
			if(strcmp(registros[y],aux) == 0){
				aux="R";
				strcat(aux,(char) y);
			}
			y++;
...
char * getId(char ** tokenizados , int i, int j){
	char *aux;
	while(tokenizados[i][j] >= 65 && tokenizados[i][j] <= 122){
			strcat(aux,tokenizados[i][j]);
			j++;
	}
	return aux;
...
char * utilizaRegistroDisponible(char * valor){
	int i = 0;
	char *r = "R";
	while(i < 8){
		if(r[i] == NULL){
...
void postSin(){
	...
	if(tokenizados[i][j] == 'M'){	
		...
		char * aux2 = getId(tokenizados, i,j);
		j++;//Saltamos ,
